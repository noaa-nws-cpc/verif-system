...
%
clear
preferenceFile = '../input/preferences_compareScores_temp_monthly_grid2deg_full_heidke.xml';
%--------------------------------------------------------------------------
...
outputFilePrefix = ['compareScores_' preferences.variable.Text '_' preferences.fcstType.Text '_' preferences.spatialType.Text '_' preferences.numDates.Text '_' preferences.scoreType.Text];
...
fprintf(fid, '# %s %6s %6s %6s\n','Valid_date','Scotts','VWT','Diff');
% Print each line of data
for i=1:length(data.file1.date)
    fprintf(fid, '  %s %6.1f %6.1f %6.1f\n', datestr(data.file1.date(i),'yyyy-mm-dd'), data.file2.score(i), data.file1.score(i), data.file1.score(i)-data.file2.score(i));
end
% Close the file
fclose(fid);
...
plot(data.file2.date, data.file2.score, 'k-', 'LineWidth',4);
% Plot VWT's scores
subplot(3,1,[1,2]), plot(data.file1.date, data.file1.score, 'r-', 'LineWidth',1);
...
% set(gca, 'FontSize', 12);
% Title
title(['Verification System Comparison - ' preferences.variable.Text ' - ' preferences.fcstType.Text ' - ' preferences.spatialType.Text ' - ' preferences.scoreType.Text]);
% Axes
xlabel('Year');
...
legend('Scotts', 'VWT', 'Location', 'Northwest');
...
corr = corrcoef(data.file1.score(not(isnan(data.file1.score))),data.file2.score(not(isnan(data.file2.score))));
...
temp = cellstr(datestr(maxAbsDiff.date,'mm/yyyy'));
temp2= cellstr(datestr(minAbsDiff.date,'mm/yyyy'));
...
annotation(gcf, 'TextBox', textPosition, 'String', textBoxString, 'FitBoxToText', 'off');
% Save PNG
